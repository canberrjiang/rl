...
def jaccard_comp(val1, val2):
    ...
    # No q-grams are generated for both values - only happens if values are too short
...
def edit_dist_sim_comp(val1, val2):
    ...
    # Faster if the first value is longer than the second value, so call function with reversed values
    ...
    # Initialise first row in the edit matrix
    previous_row = list(range(len(val2) + 1))

    for (i, ch1) in enumerate(val1):
...
def compareNumericValues(val1, val2):
    sim = 0.0
    max_diff = 0.4
    if len(val1) < 1:
        val1 = '1'
    ...
    diff_percent = float(abs(val1 - val2)) / float(max_val)
    if diff_percent < max_diff:
        sim = 1 - diff_percent

    return sim


def compareDateValues(val1, val2):
    ...
    if len(dd_1) > 2:
        dd_1 = dd_1[:2]
    elif len(dd_1) == 1 and dd_1.isnumeric() == False:
        dd_1 = dd_1 + '1'
    elif len(dd_1) == 1 and dd_1.isnumeric() == True:
        dd_1 = '0' + dd_1
    else:
        dd_1 = '01'
    ...
    if len(dd_2) > 2:
        dd_2 = dd_2[:2]
    elif len(dd_2) == 1 and dd_2.isnumeric() == False:
        dd_2 = dd_2 + '1'
    elif len(dd_2) == 1 and dd_2.isnumeric() == True:
        dd_2 = '0' + dd_2
    else:
        dd_2 = '01'

    for i in range(len(dd_1)):
        if dd_1[i].isnumeric() == False:
            dd_1 = dd_1[:i] + '0' + dd_1[i + 1:]

    for i in range(len(dd_2)):
        if dd_2[i].isnumeric() == False:
            dd_2 = dd_2[:i] + '0' + dd_2[i + 1:]

    # compare mm
    if len(mm_1) >= 2:
        mm_1 = mm_1[:2]
    elif len(mm_1) == 1 and mm_1.isnumeric() == False:
        mm_1 = mm_1 + '1'
    elif len(mm_1) == 1 and mm_1.isnumeric() == True:
        mm_1 = '0' + mm_1
    else:
        mm_1 = '01'
    ...
    if len(mm_2) >= 2:
        mm_2 = mm_2[:2]
    elif len(mm_2) == 1 and mm_2.isnumeric() == False:
        mm_2 = mm_2 + '1'
    elif len(mm_2) == 1 and mm_2.isnumeric() == True:
        mm_2 = '0' + mm_2
    else:
        mm_2 = '01'
    ...
    for i in range(len(mm_1)):
        if mm_1[i].isnumeric() == False:
            mm_1 = mm_1[:i] + '1' + mm_1[i + 1:]

    for i in range(len(mm_2)):
        if mm_2[i].isnumeric() == False:
            mm_2 = mm_2[:i] + '1' + mm_2[i + 1:]
    ...
    for i in range(len(yyyy_1)):
        if yyyy_1[i].isnumeric() == False:
            yyyy_1 = yyyy_1[:i] + '0' + yyyy_1[i + 1:]
        if i == 0 and (yyyy_1[i] not in ['1', '2']):
            yyyy_1 = yyyy_1[:i] + '1' + yyyy_1[i + 1:]
        if i == 1 and yyyy_1[0] == '1' and (yyyy_1[i] not in ['8', '9']):
            yyyy_1 = yyyy_1[:i] + '9' + yyyy_1[i + 1:]
        elif i == 1 and yyyy_1[0] == '2' and (yyyy_1[i] != '0'):
            yyyy_1 = yyyy_1[:i] + '0' + yyyy_1[i + 1:]

    for i in range(len(yyyy_2)):
        if yyyy_2[i].isnumeric() == False:
            yyyy_2 = yyyy_2[:i] + '0' + yyyy_2[i + 1:]
        if i == 0 and (yyyy_2[i] not in ['1', '2']):
            yyyy_2 = yyyy_2[:i] + '1' + yyyy_2[i + 1:]
        if i == 1 and yyyy_2[0] == '1' and (yyyy_2[i] not in ['8', '9']):
            yyyy_2 = yyyy_2[:i] + '9' + yyyy_2[i + 1:]
        elif i == 1 and yyyy_2[0] == '2' and (yyyy_2[i] != '0'):
            yyyy_2 = yyyy_2[:i] + '0' + yyyy_2[i + 1:]

    val1 = dd_1 + '-' + mm_1 + '-' + yyyy_1
    val2 = dd_2 + '-' + mm_2 + '-' + yyyy_2

    val1_date = datetime.datetime.strptime(val1, '%m-%d-%Y')
    val2_date = datetime.datetime.strptime(val2, '%m-%d-%Y')

    if val1_date > val2_date:
        delta = val1_date - val2_date
    else:
        delta = val2_date - val1_date
...
def comparePhone(val1, val2):
    val1 = val1.replace(' ', '')
    val2 = val2.replace(' ', '')
    for i in range(len(val1)):
        if val1[i].isnumeric() == False:
            val1 = val1[:i] + '0' + val1[i+1:]

    for i in range(len(val2)):
        if val2[i].isnumeric() == False:
            val2 = val2[:i] + '0' + val2[i+1:]

    sim = edit_dist_sim_comp(val1, val2)
    return sim
